...
- **Coverage badge generated by `py-cov-action/python-coverage-comment-action`**
  (supersedes the static shields.io badge above) — the manual number went stale
  between commits. The action writes `badge.svg` to an orphan
  `python-coverage-comment-action-data` branch that the README links to, so the
  badge tracks `master` automatically. Chosen over Codecov because it runs on
